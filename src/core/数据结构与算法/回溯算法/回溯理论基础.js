 ...
 * 
 * 回溯法是一种搜索方式，一般是递归函数里嵌套for循环，也就是用递归的方式去解决多个for循环嵌套的问题，然后回溯的逻辑会写在调用递归函数的位置的下面部分。
 * 
 * 回溯法的效率如何？
 ...
 * 
 * 如何理解回溯法？
 * 回溯法很抽象，但是都可以抽象成一棵n叉树。
 * n叉树的横向代表集合的大小，用for循环来遍历，而n叉树的纵向用递归遍历，因为递归就要有终止条件，所以必然是一棵高度有限的n叉树。
 * 最后，通过回溯不断调整结果集。
 * 
 * 回溯三部曲：
 * 1、递归函数参数和返回值

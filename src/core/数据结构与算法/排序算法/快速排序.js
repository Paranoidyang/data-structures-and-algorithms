...
function quickSort(arr, left = 0, right = arr.length - 1) {
  if (arr.length > 1) { // 定义递归边界，若数组只有一个元素，则没有排序必要
    const lineIndex = partition(arr, left, right) // lineIndex表示下一次划分左右子数组的索引位
    if (left < lineIndex - 1) quickSort(arr, left, lineIndex - 1) // 如果左边子数组的长度不小于1，则递归快排这个子数组，左子数组以 lineIndex-1 为右边界
    if (lineIndex < right) quickSort(arr, lineIndex, right) // 如果右边子数组的长度不小于1，则递归快排这个子数组，右子数组以 lineIndex 为左边界
  }
  return arr
}
...
function partition(arr, left, right) {
  ...
  let i = left, j = right // 初始化左右指针
  while (i <= j) { // 当左右指针不越界时，循环执行以下逻辑
    while (arr[i] < pivotValue) i++ // 左指针所指元素若小于基准值，则右移左指针
    while (arr[j] > pivotValue) j-- // 右指针所指元素大于基准值，则左移右指针

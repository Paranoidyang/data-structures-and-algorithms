/**
 * 冒泡排序 O(n^2)：从第一个元素开始，重复比较相邻的两个元素，若前者比后者大，则交换两者的位置，否则不动。
 * 每一轮操作，都会将这一轮中最大的元素放到数组的末尾。假如数组的长度是 n，那么当我们重复完 n 轮的时候，整个数组就有序了。

 * 思路：可以通过双层循环来实现，外循环控制从头到尾比较的轮数，内循环则负责两两元素比较，将元素放到正确位置上。

   冒泡排序时间复杂度在最好情况下是 O(n)，即当数组有序，且有用标识位优化过的写法时
